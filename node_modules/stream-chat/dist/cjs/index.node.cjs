...
var AttachmentManager = class {
  constructor({ composer, message }) {
    ...
    this.getAttachmentIndex = (localId) => this.attachments.findIndex(
...
var pollCompositionStateProcessors = {
  ...
  options: ({ value, data }) => {
    ...
    const shouldAddEmptyOption = prevOptions.length < MAX_POLL_OPTIONS && (!prevOptions || prevOptions.slice(index + 1).length === 0 && !!text);
    ...
    const optionListTail = shouldAddEmptyOption ? [{ id: generateUUIDv4(), text: "" }] : prevOptions.slice(index + 1);
    const newOptions = [
      ...optionListHead,
...
var createPollComposerStateMiddleware = ({
  ...
  const universalHandler = (state, validators3, processors) => {
...
var noop2 = () => void 0;
var _MessageComposer = class _MessageComposer {
  constructor({
    composition,
    config,
    compositionContext,
    ...
    this.subscribeTextComposerStateChanged = () => this.textComposer.state.subscribe((nextValue, previousValue) => {
    ...
    this.subscribeAttachmentManagerStateChanged = () => this.attachmentManager.state.subscribe((nextValue, previousValue) => {
    ...
    this.subscribeLinkPreviewsManagerStateChanged = () => this.linkPreviewsManager.state.subscribe((nextValue, previousValue) => {
      if (typeof previousValue === "undefined") return;
      this.logStateUpdateTimestamp();
      if (this.compositionIsEmpty) {
        this.deleteDraft();
    ...
    this.subscribePollComposerStateChanged = () => this.pollComposer.state.subscribe((nextValue, previousValue) => {
      if (typeof previousValue === "undefined") return;
      this.logStateUpdateTimestamp();
      if (this.compositionIsEmpty) {
        this.deleteDraft();
    ...
    this.subscribeMessageComposerStateChanged = () => this.state.subscribe((nextValue, previousValue) => {
      if (typeof previousValue === "undefined") return;
      this.logStateUpdateTimestamp();
      if (this.compositionIsEmpty) {
        this.deleteDraft();
    ...
    this.subscribeMessageComposerConfigStateChanged = () => this.configState.subscribe((nextValue) => {
...
var Channel = class {
  ...
  deleteReaction(messageID, reactionType, user_id) {
    ...
    if (user_id) {
      return this.getClient().delete(url2, { user_id });
    }
    return this.getClient().delete(url2, {});
  }
  /**
